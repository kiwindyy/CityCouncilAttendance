...
After importing the data into R (@citeR) using the Open Data Toronto package (@OpenDataToronto), it is cleaned to keep only key variables of interest and make it easier for data analysis in Section 3. To clean the data and produce graphs with the data, the following packages in R (@citeR) were used: tidyverse (@tidyverse), dplyr (@dplyr), lubridate (@lubridate), knitr (@knitr), readr (@readr) and ggplot2 (@ggplot2). 
...
#| fig-cap: Comparision of Absences by name
#| echo: false
...
ggplot(absence_by_member, aes(x = reorder(name, -absence_count), y = absence_count)) +
...
To further our understanding of the data, @fig-session-year compares the number of member absences over the years since the beginning of the term. The x-axis is the years since the beginning of the term and the y-axis counts the number of absences. 

```{r}
#| label: fig-session-year
#| fig-cap: Comparison of Member Absences by Year
#| echo: false

# Filter the data to only include rows where members were absent (present == "No")
...
ggplot(absent_data, aes(x = session_year, y = absence_count)) +
  geom_bar(stat = "identity", fill = "cyan3") +  # Use a bar plot and custom color
  labs(
    title = "# absence over years 2022-2024",
    x = "Year",
    y = "# Absences"
  ) +
  theme_minimal()
...
#| fig-cap: Comparison of Attendance by session type between 2022 to 2024
#| echo: false
...
absence_data <- cleaned_data %>%
  ...
  summarise(absence_count = n())  # Count the number of absences for each session type

# Create the plot
ggplot(absence_data, aes(x = session_type, y = absence_count)) +
  geom_bar(stat = "identity", fill = "cyan3") +  # Use horizontal bar plot and a custom color
  labs(
    title = "# absence of different mettings types",
    x = "Type",
    y = "# Absences"
  ) +
  theme_minimal()
