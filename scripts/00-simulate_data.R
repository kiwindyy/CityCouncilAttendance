...
generate_random_time_range <- function() {
  start_time <- sprintf("%02d:%02d%s", sample(1:12, 1), sample(0:59, 1), sample(c("AM", "PM"), 1))
  end_time <- sprintf("%02d:%02d%s", sample(1:12, 1), sample(0:59, 1), sample(c("AM", "PM"), 1))
...
committee_names <- c("City Council", "CreateTO", "Executive Committee", "Toronto Parking Authority", "Audit Committee")
session_dates <- as.Date(sample(seq.Date(from = as.Date("2022-01-01"), to = as.Date("2024-12-31"), by = "day"), 20, replace = FALSE))
mtgs <- c(1:4)
...
`First Name` <- split_result[seq(2, length(split_result), by = 2)]
`Last Name` <- split_result[seq(1, length(split_result), by = 2)]
Committee <- sample(committee_names, observations, replace = TRUE)
`MTG #` <- sample(mtgs, observations, replace=TRUE)
session_type <- sample(type, observations, prob=c(0.4, 0.4, 0.2), replace = TRUE)
...
sim_data <- 
  data.frame(Term=paste(begin_year, end_year, sep="-"), `First Name`, `Last Name`, Committee, `MTG #`, 
             `Session Date`=session_date, session_type, `Session Start-End Time`=session_time, Present, check.names = FALSE)
